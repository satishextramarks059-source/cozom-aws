...
class DoctorDashboard {
    ...
    async exportBookingsCSV(status) {
        ...
        try {
            const response = await fetch(`/user/api/doctor/export-bookings/?status=${status}`, {
                method: 'GET',
                headers: {
     ...
     */
    bindEvents() {
                // Reset Filter button for pending appointments
                const resetBtn = document.getElementById('resetPendingAppointmentsDateFilter');
                if (resetBtn) {
    ...
    async handleAvailabilityToggle(event) {
        ...
        try {
            ...
            if (response.ok && data.status === 'success') {
                ...
                // Update status text
                if (statusText) {
                    statusText.textContent = data.is_available ? 'Available' : 'Not Available';
                }
                ...
                this.showAlert('Failed to update availability. Please try again.', 'danger');
            }
        } catch (error) {
        ...
        if (card) {
            ...
            setTimeout(() => {
                ...
                if (container && container.children.length === 0) {
                    // Reload page to hide the appointments section
                    location.reload();
                }
            }, 300);
        }
